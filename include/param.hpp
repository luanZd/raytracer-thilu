...
public:
    ...
    // Retorna um Ponto 3D (ex: look_from="0 2 8")
    Point3 get_point3(const std::string &key, const Point3 &default_val = Point3(0.0f)) const;

    // Retorna um Vetor 3D (ex: up="0 1 0")
    Vector3 get_vector3(const std::string &key, const Vector3 &default_val = Vector3(0.0f)) const;
